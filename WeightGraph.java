...
public class WeightGraph<V> {
	...
	protected void read( String fileName ) throws Exception, IOException {
		...
		while( (line = graphFile.readLine() ) != null ) {
			...
			if( sGraph.getEdge ( sv, dv ) == null )
				...
				//Modified by Thais Bardini on November 19th, 2017 (tbard069@uottawa.ca)
			e = sGraph.insertEdge ( sv, dv, weight );
		}
	} /* read */
	...
	void printAllShortestDistances( String vert ) throws Exception {
		...
		Map<Vertex<String>, Integer> a = GraphAlgorithms.shortestPathLengths( sGraph, vSource);
		
		for(Vertex<String> i : a.keySet()) {
			System.out.println("shortest path from "+vert+" to " + i.getElement() + " is " + a.get(i));
		}
		// Find shortest path
